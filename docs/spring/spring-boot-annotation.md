...
- [2.Spring Bean](#2.Spring-Bean)
  ...
  - [2.2.`@Component`,`@Repository`,`@Service`,`@Controller`](#2.2.`@Component`,`@Repository`,`@Service`,`@Controller`)
...
最近学习使用 SpringBoot 的时候，关于注解的问题比较琐碎，就跟查字典似的。自己也是需要花时间简单总结一下了。

## 1.`@SpringBootApplication`

这里先单独提出 `@SpringBootApplication` 这个注解是 Spring Boot 项目的基石，创建 SpringBoot 项目之后会默认在主类加上。
...
根据 [SpringBoot 官网](https://docs.spring.io/spring-boot/docs/2.0.x/reference/html/using-boot-using-springbootapplication-annotation.html)，这三个注解的作用分别是：

- `@EnableAutoConfiguration`：启用 SpringBoot 的自动配置机制。
- `@ComponentScan`：扫描被 `@Component`（`@Service`,`@Controller`）注解的 bean，注解默认会扫描该类所在的包下所有的类。（[参考最佳实践](https://docs.spring.io/spring-boot/docs/2.0.x/reference/html/using-boot-structuring-your-code.html)）
- `@Configuration`：允许在 Spring 上下文中注册额外的 beans 或导入其他配置类。
